...
- [ ] Appwrite数据库搭建（cost_factors等4个Collection）
- [ ] 19国成本数据导入（38条记录：19国×2行业）
- [ ] GECOM计算引擎v2.0实现（支持用户覆盖）
- [ ] Appwrite SDK集成与数据操作封装

### 📋 待开发（MVP 2.0 - Week 2-4）
- [ ] Step 0-2界面实现（含M1-M8完整展示）⭐核心
- [ ] Step 3-4界面实现（结果+19国对比）
- [ ] Step 5 AI助手实现（工具调用+成本引擎联动）
- [ ] docx.js专业报告生成系统
- [ ] AI生成第四章战略建议（DeepSeek R1）
- [ ] 历史项目保存/加载（projects + calculations表）
- [ ] 端到端测试与部署（Appwrite Sites）
