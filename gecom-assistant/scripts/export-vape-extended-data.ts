 ...
 * Vape扩展数据导出脚本
 *
 * 目的：将本地TypeScript文件中的84个扩展字段导出为JSON文件
 * 解决：Appwrite仅存储60核心字段，本地有144字段的数据完整性问题
 *
 * 策略：三层数据架构
 * - Layer 1: TypeScript源文件 (144字段) - Git版本控制
 * - Layer 2: Appwrite数据库 (60字段) - 生产环境核心计算
 * - Layer 3: JSON扩展文件 (84字段) - 静态资源，按需加载
 *
 * 使用：npm run export:vape-extended
 */
...
function extractExtendedFields(fullData: any): any {
  const extendedData: any = {
    _metadata: {
      source_file: fullData.country + '-vape.ts',
      total_fields: 0,
      core_fields: 0,
      extended_fields: 0,
 ...
 * 导出单个国家的扩展数据
 */
async function exportCountryExtendedData(countryCode: string) {
  console.log(`\n📝 处理 ${countryCode}-vape 扩展数据...`);

  const vapeFilePath = path.join(
    process.cwd(),
    'data/cost-factors',
    `${countryCode}-vape.ts`
  );

  if (!fs.existsSync(vapeFilePath)) {
    console.log(`   ⚠️  ${countryCode}: 文件不存在，跳过`);
    return { success: false, reason: 'file_not_found' };
  }

  try {
    // 读取完整数据
    const fullData = await readTSDataFile(vapeFilePath);
    console.log(`   ✅ 读取完整数据（${Object.keys(fullData || {}).length}字段）`);

    // 提取扩展字段
    const extendedData = extractExtendedFields(fullData);
    console.log(
      `   ✅ 提取扩展字段（${extendedData._metadata.extended_fields}个）`
    );
...
async function main() {
  console.log('╔════════════════════════════════════════════════╗');
  console.log('║   Vape扩展数据导出（三层架构-Layer 3）        ║');
  console.log('╚════════════════════════════════════════════════╝');
  console.log('\n🎯 导出策略：');
  console.log('   Layer 1: TypeScript源文件 (144字段) - 单一真相来源');
  console.log('   Layer 2: Appwrite数据库 (60字段) - 核心计算查询');
  console.log('   Layer 3: JSON扩展文件 (84字段) - 详细数据按需加载\n');
  ...
  for (const country of VAPE_COUNTRIES) {
    const result = await exportCountryExtendedData(country);
    if (result.success) {
      results.success++;
  ...
  console.log('\n📂 输出位置: public/data/vape-extended/');
  console.log('📝 使用方式: fetch(\`/data/vape-extended/\${country}-vape-extended.json\`)');
  console.log('\n✅ Vape扩展数据导出完成！\n');
}
